...
class Solutions {
  ...
  // All solutions
  diameterOfBinaryTree_091225_hard(root) {
    const stack = [root];
    const seen = new Map();
    seen.set(null, [0, 0]);
    ...
    while (stack.length) {
      ...
      if (node.left && !seen.has(node.left)) {
        ...
        const [lHeight, lDiameter] = seen.get(node.left);
        const [rHeight, rDiameter] = seen.get(node.right);
    ...
    while (stack.length) {
      ...
      if (node.left && !seen.has(node.left)) {
        ...
        let diameter = Math.max(
          leftHeight + rightHeight, Math.max(leftDiameter, rightDiameter)
        );
  ...
  diameterOfBinaryTree_240625_good(root) {
    if (!root) return null;
